...
def generate_launch_description():
    ...
    # Declare the launch argument for the model (URDF or XACRO file)
    ...
    robot_description = ParameterValue(
        Command(["xacro", " ", LaunchConfiguration("model")]),  # Add space here
        value_type=str
    )
    ...
    # Node to control joint states via GUI
    joint_state_publisher_gui_node = Node(
        package="joint_state_publisher_gui",
        executable="joint_state_publisher_gui"
    )

    # RViz node to visualize the robot in RViz
    rviz_node = Node(
        package="rviz2",
        executable="rviz2",
        name="rviz2",
